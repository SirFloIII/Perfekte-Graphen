...
\begin{definition}
    Ein Tupel $G = (V, E)$ ist ein \emph{Graph}, wenn
    \begin{enumerate}
        \item $V$ ist eine Menge, genannt die \emph{Knotenmenge}. Die Elemente nennen wir \emph{Knoten}.
...
\begin{definition}
    Sei $G = (V, E)$ ein Graph. Eine (surjektive) Funktion $c: V \to C \subset \mathbb N$ heißt \emph{Färbung}\footnote{Streng genommen ist dies eine Knotenfärbung. In der weiteren Graphentheorie betrachtet man auch Kantenfärbungen, diese kommen aber in dieser Arbeit nicht vor.} von $G$, wobei wir $C$ als die Menge der \emph{Farben} bezeichnen. Die Färbung heißt \emph{zulässig} (oder \emph{gültig}), wenn keine zwei benachbarten Knoten die gleiche Farbe haben, i.e. $\forall uv \in E: c(u) \neq c(v)$.
\end{definition}

Offensichtlich ist irgendwelche zulässige Färbungen finden nicht so schwer. Als Extrembeispiel kann man jedem Knoten eine eigene Farbe zuordnen. Die wahre Herausforderung ist, eine zulässige Färbung zu finden, die möglichst wenige Farben verwendet.
...
Im Allgemeinen ist die chromatische Zahl eines Graphens finden NP-schwer \cite{Karp1972}. Eine Möglichkeit, dies denoch zu tun ist mit dem Chromatischen Polynom \todo{cite this}. Der Aufwand dafür ist aber exponentiel, konkret $O(2^{|E|})$. Für bestimmte Klassen von Graphen gibt es aber polynomielle, also wesentlich schnellere Methoden, die chromatische Zahl zu berechnen. Diese werden wir in den späteren Kapiteln dieser Arbeit kennenlernen.

\begin{satz}
    Sei $G = (V, E)$ ein Graph. Die Anzahl der zulässigen Färbungen mit $\lambda$ Farben $\chi(G, \lambda)$ nennen wir \emph{chromatisches Polynom}. $\chi(G, \lambda)$ ist tatsächlich ein Polynom und erfüllt die Rekursionsgleichung für jede Kante $e \in E$:
    \begin{equation}
...
\end{satz}
\begin{proof}
    \todo{oder auch nur zitieren?}
\end{proof}

Das wohl bekannteste Graph-Färbe-Problem, wenn auch nicht in dieser Formulierung, ist das beliebte Zahlenrätsel Sudoku. Ziel von Sudoku ist es, ein $9 \times 9$-Feld mit den Ziffern 1 bis 9 zubefüllen, sodass in keiner Zeile, Spalte und keinem Kästchen eine Ziffer mehrmals vorkommt. Wir identifizieren die Ziffern mit neun Farben und die 81 Felder als Knoten in einem Graphen. Zwischen je 2 Knoten in gleicher Zeile, Spalte und Kästchen fügen wir eine Kante hinzu. Zusätzlich fügen wir Kanten hinzu, die verschiedene gegebene Ziffern miteinander verbindet. Eine zulässige Färbung entspricht nun einer Lösung des Sudokus.
...
Die Suche nach der Cliquenzahl ist im allgemeinen ebenfalls NP-schwer \cite{Karp1972}, aber ähnlich wie bei der chromatischen Zahl werden wir für unsere speziellen Klassen effiziente Algoritmen für das Cliquenproblem kennen lernen.
...
\end{satz}
\begin{proof}
    Sei $A$ die größte Klique von $G$ und $c: V \to C$ eine minimale zulässige Färbung von G. Da die Knoten von $A$ alle untereinander verbunden sind, müssen sie von $c$ verschiedene Farben zugewiesen bekommen. Damit folgt $\Omega(G) = |A| \leq |C| = \chi(G)$.
\end{proof}

...
Offensichtlich ist die Eigenschaft \emph{Vergleichbarkeitsgraph} zu sein ebenfalls hereditär. Zu gegebener Knotenmenge $A \subset V$ ist $(A, F \cap (A \times A))$ eine Orientierung von $G_A$. Es reicht also wieder nur $\chi(G) = \omega(G)$ um die Perfektheit im nachfolgenden Teil zu zeigen.

\begin{bemerkung}
    Orientierungen von Vergleichbarkeitsgraphen sind nicht mit Hasse-Diagrammen zu verwechseln. Während in den Orientierungen explizit die Transitivität gefordert wird, werden in Hasse-Diagrammen Kanten, die wegen der Transitivität impliziert werden, weggelassen. (Vgl. Abbildung \ref{fig:hasse})
\end{bemerkung}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        v &\mapsto \begin{cases}
            0 &v \text{ ist eine Quelle}\\
            \max\limits_{wv \in F} h(w) + 1 &\text {sonst}
        \end{cases}
    \end{align*}
    
    Aufgrund der Transitivität und der Antisymmetrie ist der Graph $(V, F)$ zyklenfrei. Weil $V$ endlich ist gibt es mindestens eine Quelle, i.e. einen Knoten, der keine eingehenden Kanten hat. Damit ist $h$ wohldefiniert. Definiere $m := \max_{v \in V} h(v)$.
    ...
    Zu einem solchen Knoten $v$, bei dem $h(v) = m$ maximal ist, findet man einen Pfad $v =: [u_m, \hdots, u_0]$ von $v$ zu einer Quelle $u_0$ mit $h(u_i) = i\, \forall i \in \N_{\leq m}$, indem man einfach in jedem Schritt einen Zeugen des Maximums in der Definiton von $h$ als nächsten Knoten wählt. Aufgrund der Transitivität von $F$ ist der Pfad $[u_m, \hdots, u_0]$ sogar eine $m+1$ große Clique.
    
    Es kann also keine größere Clique und keine kleinere Färbung als diese geben und $\chi(G) = m+1 = \omega(G)$. Wegen der Hereditärität ist $(V, E)$ also perfekt.
\end{proof}

\begin{korollar}
    Sei $(V, E)$ ein Vergleichbarkeitsgraph und sei eine Orientierung $(V, F)$ gegeben. Dann kann eine minimale zulässige Färbung $h : V \to \N$, sowie eine maximale Clique in linearer Zeit, also $O(|V| + |F|)$ gefunden werden.
\end{korollar}
\begin{proof}
...
def compute_height(G: DirectedGraph) -> dict[Vertex, int]:
    ...
    Der Algorithmus läuft über eine Tiefensuche \emph{(eng. Depth-First-Search, DFS)}, wobei $h$ für einen Knoten erst berechnet wird, wenn $h$ für alle Ausgang-Nachbarn bereits berechnet ist. In \verb|unset_inneighbor_count[u]| wird absteigend gezählt, wie oft die Funktion \verb|DFS| für einen Knoten \verb|u| aufgerufen wurde. Sobald \verb|DFS| von jedem seiner Ausgang-Nachbarn aufgerufen wurde, erkannt durch \verb|unset_inneighbor_count[u] == 0|, dass $h$ für alle Ausgang-Nachbaren von \verb|u| berechnet wurde.
    
    \verb|DFS| wird pro Kante einmal aufgerufen und pro Knoten $u$ wird einmal der Teil innerhalb der \verb|if|-Abfrage ausgeführt (falls $u$ keine Quelle ist):
    \begin{itemize}
        \item $O(Ingrad(u))$ für das Maximum
        \item $O(Ausgrad(u))$ für das rekursive Aufrufen von \verb|DFS|
    \end{itemize}
    
    Die Vorbereitung von \verb|unset_inneighbor_count| sowie das Durchlaufen der Knoten auf der Suche nach den Quellen braucht jeweils $O(|V|)$. Insgesamt braucht der Algorithmus also $O(2\cdot|V| + 3\cdot|F|) = O(|V| + |F|)$ viel Zeit.

    Mit dem Algorithmus \ref{algo:clique} kann die maximale Clique in linearer Zeit gefunden werden. 
...
def find_path(G: DirectedGraph) -> list[Vertex]:
    ...
    return path
        \end{minted}
        \caption{Pseudo-Pseudo-Code zur Berechnung einer Clique. \todo{Remove Joke}}
    \end{listing}
